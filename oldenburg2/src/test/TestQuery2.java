...
public class TestQuery2 {
	...
	public static void main(String args[]) throws Exception {
		...
		*/
		...
		while ((allpaths = h.deleteMin()) != null) {
			...
			/*System.out.println("Sky line : "+ allpaths.path.size());
	...
	public static List<List<Node<Vertex>>> findsmallpath(Node<Vertex> n, Node<Vertex> lastn, Node<Vertex> nextn,Cluster c) {
		...
		if (!n.isChildren()) {
			...
			/*map Contain Linked
			if(findMapFlg1 == true && findMapFlg2 == true){
				Node<Vertex> Level1Child = findLevel1ChildrenByID(n);
				if(n != null){
					if(isContainLinked(mapLastId, Level1Child) && isContainLinked(mapNextId, Level1Child) ){
						LinkedList<Node<Vertex>> l = new LinkedList<Node<Vertex>>();
						l.add(Level1Child);
						out.add(l);
						System.out.println("isContainLinked =");
						return out;
					}
				}
			}
			else if(findMapFlg1 == true){
				Node<Vertex> Level1Child2 = findLevel1ChildrenByID(n);
				if(n != null){
					if(isContainLinked(mapLastId, Level1Child)){
						System.out.println("isContainLinked 0");
						//findMapFlg1 = false;
					}
				}
			}
			else if(findMapFlg2 == true){
				Node<Vertex> Level1Child2 = findLevel1ChildrenByID(n);
				if(n != null){
					if(isContainLinked(mapNextId, Level1Child)){
						System.out.println("isContainLinked 1");
						//findMapFlg2 = false;
					}
				}
			}	
						...
						if (skyArray[skyArray.length - 1].vertex.vertexID == mapNextId) {
							...
							for(Node<Vertex> node : outArray){
								if(node.vertex.vertexID == n.vertex.vertexID){
									...
									if(isContainLinked(mapLastId, node) && isContainLinked(mapNextId, node) ){
										LinkedList<Node<Vertex>> l = new LinkedList<Node<Vertex>>();
										l.add(node);
										//out.add(l);
										//System.out.println("isContainLinked =");	
		...
		for (int i = 0; i < out.size(); i++) {
			List<Node<Vertex>> list = out.get(i);
			for(Node<Vertex> nn :list){
				System.out.print(nn.vertex.vertexID+" ");
			}
			System.out.println();
		}*/
		return out;
	...
	public static List<List<Node<Vertex>>> findsmallpath(boolean starflg, Cluster c, int objectID, Node<Vertex> n,
		...
		if (!n.isChildren()) {
			...
			if(findMapFlg == true){
				Node<Vertex> Level1Child = findLevel1ChildrenByID(n);
				if(n != null){
					if(isContainLinked(linkedn.vertex.vertexID, Level1Child)){
						LinkedList<Node<Vertex>> l = new LinkedList<Node<Vertex>>();
						l.add(Level1Child);
						out.add(l);
						System.out.println("isContainLinked 2");
						//findMapFlg = false;
					}
				}
			}*/
...
public static Node<Vertex> findLevel1ChildrenByID(Node<Vertex> n) {
		for (int i = 0; i < n.children.size(); i++) {
			Node<Vertex> node = n.children.get(i);
			if(node.vertex.vertexID == n.vertex.vertexID){
				return node;
			}
		}
